...
import { AjaibStockMarketSyncService } from './jobs/stock-sync/sync/ajaib-stock-market-sync.service';
...
async function bootstrap() {
    ...
    try {
        ...
        logger.error(
            `Ajaib stock market sync failed: ${error instanceof Error ? error.message : String(error)
            }`,
        );
        process.exitCode = 1;
    } finally {
